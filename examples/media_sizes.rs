use cups_rs::{MEDIA_FLAGS_DEFAULT, MEDIA_FLAGS_DUPLEX, MEDIA_FLAGS_READY, MEDIA_LETTER};
use cups_rs::{MediaSize, Result, get_destination};
use std::ptr;
...
fn main() -> Result<()> {
    ...
    let printer_name = std::env::args().nth(1).unwrap_or_else(|| "PDF".to_string()); // Default to "PDF" if no argument
    ...
    println!("\nDefault media size:");
    match info.get_default_media(ptr::null_mut(), destination.as_ptr(), MEDIA_FLAGS_DEFAULT) {
    ...
    match info.get_all_media(ptr::null_mut(), destination.as_ptr(), MEDIA_FLAGS_READY) {
        Ok(media_list) => {
...
fn print_media_details(media: &MediaSize) {
    ...
    println!(
        "  Printable area: {:.2}\" x {:.2}\"",
        media.printable_width_inches(),
        media.printable_length_inches()
    );
    println!("  Margins:");
