...
pub fn cups_error_to_our_error(operation: &str, dest_name: Option<&str>) -> Error {
    ...
    match code {
        ...
        401 => Error::AuthenticationRequired(
            dest_name.unwrap_or("unknown").to_string()
        ),
        ...
        404 => Error::DestinationNotFound(
            dest_name.unwrap_or("unknown").to_string()
        ),
        
        503 => Error::PrinterNotAccepting(
            dest_name.unwrap_or("unknown").to_string(),
            message.clone()
        ),
...
pub fn validate_document_format(format: &str, dest_name: &str) -> Result<(), Error> {
    ...
    if !supported_formats.contains(&format) {
        return Err(Error::InvalidFormat(format.to_string(), dest_name.to_string()));
    }
